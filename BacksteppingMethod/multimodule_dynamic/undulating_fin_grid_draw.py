...
def generate_undulating_fin_mesh_right(L_x, t, resolution=[30,30], freq=1, n = 1 , amp_angl=np.pi/6,plot_flag=False):
    ...
    if plot_flag is True:
        ...
        for j in range(resolution[1]):
            ax.scatter(vertices[:,j,0], vertices[:,j,1], vertices[:,j,2])
        # 绘制法向量
        ...
        ax.set_ylabel('Y')
        ax.set_zlabel('Z')
        ax.set_xlim(-0.01, 0.3)  # 设置x轴范围一致
        ax.set_ylim(-0.01, 0.3)  # 设置x轴范围一致
        ax.set_zlim(-0.2, 0.4)  # 设置x轴范围一致
        # plt.show()
        plt.pause(0.01)
...
for t in np.arange(0, 10, dt):
    ...
    fin_vers, fin_tris, fin_norms, fin_areas, fin_vels = generate_undulating_fin_mesh_right(L_x, t, [Nx, Ny], freq, n, plot_flag=True)
    ...
    # print(t, 'lift force:', 2 *500 * CL * drag_force_avg)
