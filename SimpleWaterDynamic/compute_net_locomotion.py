...
LastEditTime: 2024-12-04 16:03:40
'''
import numpy as np
...
# 构建鱼的几何
body_vertices, body_triangles = tools.generate_ellipsoid(1, 0.5, 0.5, resolution=100)
tail_vertices, tail_triangles = tools.generate_plate(2, 0.1, 0.5, resolution=30)
...
body.compute_inertia_matrix(tools.compute_ellipsoid_inertia(0.1, 0.5, 1, 0.5), 0.1)
...
for t in np.arange(0, 10 * np.pi, dt):
    ...
    freq = 2
    joint_angle = 0.2 * np.sin(2 * np.pi *freq* t)
    d_joint_angle = 0.7 * np.sin(2 * np.pi *freq* (t+dt)) - 0.2 * np.sin(2 * np.pi *freq* t)
    ...
    position_xs.append(assembly.objects[0]._SE3[3,0])
    position_ys.append(assembly.objects[0]._SE3[3,1])
    # 可视化结果
